...
DROP POLICY IF EXISTS "Users can delete own live streams" ON live_streams;
...
DROP POLICY IF EXISTS "Anyone can view live stream viewers" ON live_stream_viewers;
CREATE POLICY "Anyone can view live stream viewers"
ON live_stream_viewers
FOR SELECT
USING (true);

DROP POLICY IF EXISTS "Users can join live streams" ON live_stream_viewers;
CREATE POLICY "Users can join live streams"
ON live_stream_viewers
FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can leave live streams" ON live_stream_viewers;
